...
- `type`：目标类型，`center`、`entrance` 或 `generic`。
- `min_radius_px` / `max_radius_px`：圆半径范围，单位为原始画面像素。
- `min_confidence`：最低置信度。
- `max_results`：最多显示结果数量；中心喇叭口通常设置为 `1`。
...
- `min_rim_edge_support`：圆周边缘支撑度，中心喇叭口误识别控制的关键参数。
...
USB 摄像头第一次调试要先确认实际分辨率。如果实际不是 1920x1080，需要按实际画面重新设置 ROI。
...
1. 缩小 `roi`。
2. 设置 `require_circle_inside_roi=true`。
3. 提高 `min_rim_edge_support`。
4. 提高 `hough_accumulator_threshold`。
5. 提高 `min_confidence`。
6. 缩小 `max_radius_px` 或 `max_radius_image_ratio`。
...
当 `save_debug_stages=true` 时，实时窗口按 `s` 会保存：
